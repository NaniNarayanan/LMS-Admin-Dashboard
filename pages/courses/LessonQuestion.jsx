...
import { MDBCol, MDBIcon } from "mdb-react-ui-kit";
 ...
 const LessonQuestion = () =>{
  ...
  const lessonid = location.pathname.split('/')[6]
  const moduleid = location.pathname.split('/')[8] 
  const courseid =   location.pathname.split('/')[9]
  const coursename =   location.pathname.split('/')[10]
  const modulename =   location.pathname.split('/')[11]
  const subtopicid = location.pathname.split('/')[7]
  const subtopicname = location.pathname.split('/')[12]
  const lessonname = location.pathname.split('/')[13]
...
const handleSubmit = (e) => {
  ...
  axios.post('http://localhost:8081/api/lessonquestions', newQuestion)
    .then(() => {
...
const deleteModule = (id) =>{
  ...
  const columns = [
    ...
    return(
        <div className="courses">
            ...
            <div style={{textAlign:"center"}}>
              <h1 style={{letterSpacing:"3px"}}>SubTopic MCQ Management</h1>
    ...
    <Modal show={show}  onHide={handleClose} size="lg">
        ...
        <label htmlFor="question">LessonId:</label>
        <input
